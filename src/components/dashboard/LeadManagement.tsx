...
export function LeadManagement() {
  ...
  const { data: leads, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // For Lead Management page, only show leads assigned to current user (excluding Hibernation, L2, and L6)
      else if (isLeadManagementPage && user) {
        query = query.eq('assigned_to', user.id).neq('status', 'hibernation').neq('status', 'L2-Call reschedule').neq('status', 'L6-Appointment set');
      }
  ...
  useEffect(() => {
    ...
    const checkExpiredLeads = async () => {
      ...
      // Find leads assigned to current user that have expired (excluding L2 scheduled callbacks and L6 appointment set)
      const { data: expiredLeads } = await supabase
        .from('leads')
        .select('id')
        .eq('assigned_to', user.id)
        .neq('status', 'L2-Call reschedule')
        .neq('status', 'L6-Appointment set')
